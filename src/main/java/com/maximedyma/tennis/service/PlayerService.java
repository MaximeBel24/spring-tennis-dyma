...
public class PlayerService {
    ...
    public Player getByIdentifier(UUID identifier) {
        log.info("Invoking getByLastName with identifier={}", identifier);
        try {
    ...
    public Player create(PlayerToCreate playerToCreate) {
        ...
        try {
            ...
            if (player.isPresent()) {
                log.warn(
                        "Player to create with firstName={}, lastName={} and birthDate={} already exists",
                        playerToCreate.firstName(),
                        playerToCreate.lastName(),
                        playerToCreate.birthDate()
                );
            ...
            return getByIdentifier(registeredPlayer.getIdentifier());
        } catch (DataAccessException e) {
    ...
    public Player update(PlayerToUpdate playerToUpdate) {
        ...
        try {
            ...
            if (existingPlayer.isEmpty()) {
                log.warn("Could not find player to update with lastName={}", playerToUpdate.identifier());
            ...
            if (potentiallyDuplicatedPlayer.isPresent() && !potentiallyDuplicatedPlayer.get().getIdentifier().equals(playerToUpdate.identifier())) {
                log.warn(
                        "Player to update with firstName={}, lastName={} and birthDate={} already exists",
                        playerToUpdate.firstName(),
                        playerToUpdate.lastName(),
                        playerToUpdate.birthDate()
                );
    ...
    private void getPlayerNewRanking() {
